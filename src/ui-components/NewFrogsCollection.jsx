...
import { listFrogs } from "../graphql/queries";
...
export default function NewFrogsCollection(props) {
    ...
    while ((newCache.length < cacheUntil || !isPaginated) && newNext != null) {
      ...
      const result = (
        await client.graphql({
          query: listFrogs.replaceAll("__typename", ""),
          variables,
        })
      ).data.listFrogs;
  ...
  return (
    <div>
      <Collection
        ...
        {...getOverrideProps(overrides, "NewFrogsCollection")}
        {...rest}
      >
        {(item, index) => {
          if (loading) {
